...
namespace Day08
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (charTracker < width && !done)
            {
                char toWrite = '3';
                for (int i = 0; i < Layers.Count; i++)
                {
                    char temp = Layers[i][layerTracker][charTracker];

                    if (toWrite == '3' || toWrite == '2')
                    {
                        toWrite = temp;
                    }
                }
                File.AppendAllText("text.txt",toWrite.ToString());
                charTracker++;
                if (charTracker % width == 0)
                {
                    charTracker = 0;
                    layerTracker++;
